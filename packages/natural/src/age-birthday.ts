 ...
 *
 * Note: The vocabulary constants here (LIFE_STAGE_AGES, MONTH_NAMES, etc.)
 * should be kept in sync with @edtf-ts/utils/age-constants.ts for consistency
 ...
 * Keep in sync with @edtf-ts/utils/age-constants.ts
 */
...
/**
 * Life stage vocabulary with age ranges.
 * Keep in sync with @edtf-ts/utils/age-constants.ts
 */
const LIFE_STAGE_AGES: Record<string, [number, number | null]> = {
  newborn: [0, 0],
  infant: [0, 1],
  toddler: [1, 3],
  preschooler: [3, 5],
...
/**
 * Teen subdivisions for more specific age ranges.
 */
const TEEN_SUBDIVISIONS: Record<string, [number, number]> = {
  'early teens': [13, 15],
  'early teen': [13, 15],
  'mid teens': [16, 17],
  'mid teen': [16, 17],
...
function parseAgeExpression(input: string): AgeExpression | null {
  ...
  // Try teen subdivisions first (more specific)
  for (const [phrase, [min, max]] of Object.entries(TEEN_SUBDIVISIONS)) {
  ...
  // Try life stages
  for (const [stage, [min, max]] of Object.entries(LIFE_STAGE_AGES)) {
...
export function parseAgeBirthday(
  ...
  const hasAgeIndicator = /\b(\d+\s*(years?|yo|y\/o|months?|weeks?|days?)|age\s+\d+|teens?|teenager|adolescent|newborn|infant|toddler|preschooler|child|school-age|pre-teen|preteen|tween|middle-schooler|young\s+adult|adult|middle-aged|senior|elderly|twenties|thirties|forties|fifties|sixties|seventies|eighties|nineties|\d0s)\b/i.test(normalized);
